...
function drawTgFractal() {
  ...
  for (let x = 0; x < width; x++) {
    for (let y = 0; y < height; y++) {
      ...
      // Використовуємо HSL для кольорової палітри
      if (iteration === maxIterations) {
        ...
        const rgb = hslToRgb(hue / 360, 1, 0.5); // функція перетворення HSL->RGB
...
function drawFixedAxes() {
  ctx.save();
  ctx.strokeStyle = "#0a1f1f";
  ctx.fillStyle = "#0a1f1f";
  ctx.lineWidth = 2;
  
  // Горизонтальна вісь (Re)
  ctx.beginPath();
...
function drawKochSnowflake(iterations, size) {
  // Очищення канвасу і малювання осей з масштабуванням
  drawAxes();
  ctx.save();
  ctx.translate(centerX, centerY);
  ctx.scale(scale, scale);
  
  // Початковий еквілатеральний трикутник з координатами, де крок поділок = 10
  ...
  function kochLine(pA, pB, iter) {
    ...
    const pPeak = { x: pA1.x + Math.cos(angle) * length, y: pA1.y + Math.sin(angle) * length };
...
function updateParamsPanel() {
  ...
  if (fractalType === "tg") {
    ...
    html += `<label for="scaleFractal">Масштаб (область):</label><input type="number" id="scaleFractal" step="0.1" value="4">`;
    ...
    html += `<label for="kochSize">Довжина сторони:</label><input type="number" id="kochSize" value="300">`;
  }
